...
import {pgnReader} from '../../pgn-reader/pgn';
...
let pgnBase = function (boardId, configuration) {
    ...
    let defaults = { theme: "blue",pieceStyle: 'merida',  manyGames: false,
                    showCoords: true, orientation: 'white', position: 'start',
                    showFen: false, layout: 'top', headers: true,
                    timerTime: 700, locale: 'en', movable: {free: false},
                    highlight: {lastMove: true}, viewOnly: true,
                    hideMovesBefore: false, colorMarker: null, showResult: false,
                    timeAnnotation: 'none', notation: 'short', notationLayout: 'inline',   // Possible: inline, list, allList
        IDs: {
    ...
    console.log ({configuration:that.configuration})
    that.mypgn = pgnReader(that.configuration);
    console.log ({parsedPgn:that.mypgn.getMoves()})
    
    let game = that.mypgn.game;     // Use the same instance from chess.src
    console.log ({possibleMoves:that.mypgn.possibleMoves(game)})
    ...
    that.configuration.i18n = i18next("en")
    ...
    function createEle(kind, id, clazz, my_theme, father, fontFamily, buttonName) {
        let ele = document.createElement(kind);
        if (id) {
        ...
        if (clazz) {
            if (my_theme) {
                if (fontFamily){
                    ele.setAttribute ("style", `font-family: ${fontFamily}`)
                    ele.innerHTML = buttonName;                   // Insert text
                }
    ...
    const generateHTML = function () {
        ...
        function addButton(pair, buttonDiv) {
            ...
            let buttonName = pair[0]==="next"? "X" : pair[0]==="play"? "G": pair[0]==="prev"? "Y":pair[0]==="flipper"? "B":pair[0]==="last"? "V":
            pair[0]==="first"? "W" :   "Z"
            const button = createEle("i", id('buttonsId') + pair[0], "button fa " + pair[0] + " " + pair[1], l_theme, buttonDiv, "lichess", buttonName);
        ...
        const resizeLayout = function () {
            console.log("resissee computing layout")
            if (hasMode('board')) {
